...
export const rateTvShow = async (tvShowId: number, rating: number) => {
    const res = await fetch(`https://api.themoviedb.org/3/movie/${tvShowId}/rating?/guest_session_id=${localStorage.getItem("guest_session_id")}&api_key=${process.env.REACT_APP_API_KEY}`,
        {
            headers: {
                accept : "application/json",
                "content-type": "application/json;charset=utf-8"
